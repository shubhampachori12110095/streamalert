...
class StreamAlertFirehose(object):
    ...
    def __init__(self, region, firehose_config, log_sources):
        ...
        # Expand disabled logs into specific subtypes
        self._disabled_logs = self._load_disabled_log_sources(firehose_config, log_sources)
    ...
    @staticmethod
    def _load_disabled_log_sources(firehose_config, log_sources):
        """Load and expand all declared disabled Firehose log sources
        ...
        """
        disabled_logs = set()
        for disabled_log in firehose_config.get('disabled_logs', []):
            disabled_log_parts = disabled_log.split(':')

            # Expand to all subtypes
            if len(disabled_log_parts) == 1:
                expanded_logs = [log_name for log_name in log_sources
                                 if log_name.split(':')[0] == disabled_log_parts[0]]
                # If the list comprehension is Falsey, it means no matching logs
                # were found while doing the expansion.
                if not expanded_logs:
                    LOGGER.error('Disabled log %s not declared in logs.json', disabled_log)

                disabled_logs.update(expanded_logs)

            elif len(disabled_log_parts) == 2:
                if disabled_log not in log_sources:
                    LOGGER.error('Disabled log %s not declared in logs.json', disabled_log)

                disabled_logs.add(disabled_log)
    ...
    def enabled_log_source(self, log_source_name):
        ...
        """
        return log_source_name not in self._disabled_logs
