...
class TestStreamAlertFirehose(object):
    ...
    def test_record_delivery_failed_put_count(self, mock_logging):
        ...
        self._mock_delivery_streams(['streamalert_data_test_log_type_json_nested',
    ...
    @mock_kinesis
    def test_load_disabled_sources(self):
        """StreamAlertFirehose - Load Disabled Sources"""
        config = load_config('tests/unit/conf')
        firehose_config = {'disabled_logs': ['json:regex_key_with_envelope',
                                             'test_cloudtrail',
                                             'cloudwatch']} # expands to 2 logs

        sa_firehose = StreamAlertFirehose(region='us-east-1',
                                          firehose_config=firehose_config,
                                          log_sources=config['logs'])

        assert_equal(len(sa_firehose._disabled_logs), 4)
        assert_true(sa_firehose.enabled_log_source('test_inspec'))

    @patch('stream_alert.rule_processor.firehose.LOGGER')
    @mock_kinesis
    def test_load_disabled_sources_invalid_log(self, mock_logging):
        """StreamAlertFirehose - Load Disabled Sources - Invalid Log"""
        config = load_config('tests/unit/conf')
        firehose_config = {'disabled_logs': ['log-that-doesnt-exist']}

        sa_firehose = StreamAlertFirehose(region='us-east-1',
                                          firehose_config=firehose_config,
                                          log_sources=config['logs'])

        assert_equal(len(sa_firehose._disabled_logs), 0)
        assert_true(mock_logging.error.called)
